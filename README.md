...
Feishu Bitable 按钮 → Automation 立即写入 `提交人`/`提交时间`、置 `已请求=true`，并将可提交状态改为 `待处理` → worker 轮询 → **基础数据 JSON** 写入 `preset-db` 并开 PR → 回写 `状态=审核中`。
...
```
「提交审核」按钮 ──> Automation 写 提交人/提交时间 + 置「已请求」= true + 可提交状态改为「待处理」 ──> worker 每 N 秒轮询 ──> 处理
```

按钮自动化先把请求即时显示为「待处理」,作为已接收/排队反馈。仅当点击前状态为空/`草稿`/`失败`/`已通过`/`已拒绝`时改成「待处理」；若当前为`待处理`/`处理中`/`审核中`,保留状态，避免重复点击把正在处理或等待审核的行退回队列。无论分支如何，都写入提交人/提交时间并置 `已请求=true`。worker 仍单实例、顺序处理；排队行会保持「待处理」，轮到后才变为「处理中」。
...
| `提交审核` | 按钮 | 触发入口,需在 Automation 中配置提交人/提交时间/已请求；可提交状态立即置「待处理」，在途状态保留 |
